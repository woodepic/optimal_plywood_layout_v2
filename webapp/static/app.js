...
// must never confuse. Minimum separation is dE 26 over ANY pair and dE 32 between
// neighbours. The original golden-angle hue ramp put some pairs at dE ~0, which is how
// two different widths came to look identical.
//
...
const WIDTH_PALETTE = ['#8a4242','#10f910','#1010f9','#caf910','#ab25e4','#56b616','#1056f9','#f9ca10','#0553c7','#a6b616','#f910ca','#10f99c','#f9106d','#10f9f9','#f91010','#4c92fa','#d9fc88','#712e9e','#16b686','#b61626','#05b4c7','#c76605','#c288fc','#5f8a42','#9e2e71','#acd7b9','#42498a','#9e7c2e','#88cdfc','#fc8888','#428a74','#f292d5','#42748a','#f2c292','#aea0e3','#d7acb9'];
...
function sheetCard(sh, pos) {
  ...
  for (const st of sh.strips) {
    ...
    const fill = widthColor(st.color);
    svg.append(sv('rect', { x: 0, y: st.y, width: W, height: st.width,
      fill, opacity: 0.2 }));
    for (const p of st.parts) {
      // Full-strength fill: washing it out to 0.62 over a tinted band collapsed
      // colours that the palette had deliberately separated.
      ...
      if (p.len > 300 && p.w > 90) {
        const t = sv('text', { x: p.x + p.len / 2, y: p.y + p.w / 2 + 14,
          'text-anchor': 'middle', 'font-size': 40, fill: labelInk(fill) });
