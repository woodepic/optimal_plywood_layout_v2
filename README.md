...
Strips are coloured **by width** — equal widths share a colour, because equal widths
share a track-saw stop setting. The palette is generated by farthest-point sampling in
CIE Lab and then reordered so *consecutive* indices are maximally far apart; since
widths are indexed in sorted order, the numerically closest widths always get the most
different colours. Minimum separation is dE 26 over any pair, dE 32 between neighbours.
Part labels pick black or white ink from the fill's luminance.
