...
Upload a STEP file, set costs, choose a time budget, Solve. Sheets are laid out in a
grid you **scroll to zoom and drag to pan**; zoom is anchored at the cursor, so the point
under the pointer stays put. It auto-fits on load, and the column count is selectable.

Every part is coloured by **its own width across the strip** — the rip dimension — so
two parts of the same colour are the same width and differ only in length. Colouring by
the *strip's* rip width instead painted a whole strip one colour, which under non-exact
2-stage hid every distinction inside it: a sheet carrying six different widths showed
two colours.

Part labels read **width x length in that same order**, matching the colour. The
canonical part dimensions read the other way round for a rotated part, so a
6-7/8 x 22-3/4 panel lying 22-3/4 across its strip looked as though it had the wrong
colour until the label was changed to describe the piece as placed.

The palette is generated by farthest-point sampling in CIE Lab and then reordered so
*consecutive* indices are maximally far apart. Widths are indexed in sorted order, so
the numerically closest widths get the most different colours — this job has widths
1/32" apart (23-1/8" and 23-3/32"). 48 colours, minimum separation dE 25.5 over any
pair. Labels pick black or white ink from the fill's luminance, since the palette spans
a wide lightness range. The faint band behind each strip keeps the *rip* width's colour,
so which strips share a stop setting is still visible.

Every part is labelled, however small. The old rule skipped anything under 9-3/8" long
or 2-13/16" wide, which dropped the dimensions off exactly the narrow slices that are
hardest to identify by eye; text now turns to run along the part's long axis and is
sized to fit, and the grid zooms if a label is still tight.

Cut lines are coloured by which saw makes them: solid orange for a track-saw rip, dashed
blue for a mitre crosscut, dashed orange for a crosscut on a strip too wide for the mitre
saw, dotted purple for a trim rip.

**Hatching marks every square inch of waste** — the strip-end offcut, the sliver beside a
part narrower than its strip, and the band left over after the last rip. Part area plus
waste plus kerf equals the sheet exactly, and a test enforces that, so nothing you are
paying for is invisible.

Records are kept automatically, per model, keyed by the SHA of the STEP file's bytes.
Whenever a solve beats the cheapest layout previously found for that file, a **new**
record is written — never an overwrite, because the cost model that made the old one a
record may be the one you come back to. Records show two prices: repriced under the costs
now in the form, and what they cost under the costs they were found with. A `*` marks a
record found under different costs, naming which knobs differ.
